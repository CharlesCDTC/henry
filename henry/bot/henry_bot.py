...
def summarize_results(actions, pnl):
    ...
    return f"Buys: {buys}, Sells: {sells}, Net P&L: {pnl:.2f}%"

def explain_strategy(symbol, timeframe, strategy):
    return f"Strategy: {strategy.upper()} on {symbol} ({timeframe}). Based on PPO, RSI, and MACD signals."
